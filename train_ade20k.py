...
def run_train(gpu, args, port):
    ...
    dataset = ADEPointDataset(args.train_image_root, args.train_data_list, args.train_gt_root, train_size,
...
def run_test(gpu, args, port, verbose=False):
    ...
    # data
    dataset = ADEPointDataset(args.test_image_root, None, args.test_gt_root, None, return_src=True)
    ...
    get_pred_src = lambda x: os.path.join(args.snapshot, 'results', 'prediction', os.path.basename(x).rsplit('.', 1)[0] + '.png')
    ...
    if (gpu == 0):
        logger = getLogger(args.snapshot, args.model)
        compute_ade_iou(os.path.join(args.snapshot, 'results', 'prediction'), args.test_gt_root, logger=logger)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--snapshot', type=str, default='./snapshot/baseline/ade20k')
