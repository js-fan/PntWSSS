...
from core.data.ADE20k import ADEPointDataset
...
from compute_iou import compute_ade_iou
...
def run_train(gpu, args, port):
    ...
    dataset = ADEPointDataset(args.train_image_root, args.train_data_list,
            args.train_gt_root, train_size,
    ...
    if is_log:
        ...
        get_segment_map = lambda x: ADE.palette[x.ravel()].reshape(x.shape+(3,))
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch, return_vals in enumerate(loader, 1):
            image, label, superpixel, label_class = return_vals
            optimizer.zero_grad()
            ...
            has_mem = (args.lambda_mem > 0) and all([deq_result[0] is not None for deq_result in deq_results])
            ...
            # backward
            upper_iter = num_batch * args.rampup
            ...
            if is_log and (batch % args.log_frequency == 0):
                ...
                examples_ = [v_img, v_gt, v_pred, v_seed_from_sp]
...
def run_test(gpu, args, port, verbose=False):
    ...
    # data
    dataset = ADEPointDataset(args.test_image_root, None, args.test_gt_root,
    ...
    if (gpu == 0):
        ...
        compute_ade_iou(
                ...
                args.test_gt_root, logger=logger)
...
if __name__ == '__main__':
    dataset_root = '/home/junsong_fan/diskf/data/ADE20k'

    parser = argparse.ArgumentParser()
    parser.add_argument('--train-image-root', type=str, default=dataset_root+'/training')
    parser.add_argument('--train-gt-root', type=str, default='')
    parser.add_argument('--test-image-root', type=str, default=dataset_root+'/validation')
    parser.add_argument('--test-gt-root', type=str, default=dataset_root+'/ADE_validation_annotations')
    parser.add_argument('--train-data-list', type=str, default=dataset_root+'/scripts/ADE20k_points.txt')
    parser.add_argument('--test-data-list', type=str, default=dataset_root+'/scripts/ADE20k_val.txt')
    parser.add_argument('--superpixel-root', type=str, default=dataset_root+'/training_scg_png')

    parser.add_argument('--model', type=str, default='deeplab_v2')
    parser.add_argument('--train-size', type=str, default='321,321')
    parser.add_argument('--test-scales', type=str, default='0.75,1,1.25')
    parser.add_argument('--num-classes', type=int, default=150, help='classification branch num_classes')
    ...
    parser.add_argument('--snapshot', type=str, default='./snapshot/ablation/ade20k')
    parser.add_argument('--num-workers', type=int, default=16)
    parser.add_argument('--gpus', type=str, default='0,1,2,3')
    parser.add_argument('--log-frequency', type=int, default=60)
    ...
    parser.add_argument('--rampup', type=float, default=35)
