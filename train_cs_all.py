...
def run_train(gpu, args, port):
    ...
    dataset = CSPointDataset(args.train_image_root, args.train_data_list, args.train_gt_root,
            train_size,
            rand_crop=True, rand_mirror=True, rand_scale=(0.25, 0.75) if args.small_image else (0.5, 1.5),
    ...
    if is_log:
        ...
        meter = AvgMeter()
        saver = SaveParams(mod, args.snapshot, args.model, num_save=30)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch, return_vals in enumerate(loader, 1):
            ...
            with torch.no_grad():
                ...
                sp_feat_label[sp_feat_prob < 0.95] = 255
                ...
                img_feat_label[img_item_cnt < 30] = 255
            ...
            if has_mem:
                ...
                for deq_feat, deq_label in deq_results:
                    ...
                    # pos, neg masks
                    with torch.no_grad():
                        if args.use_pred_region:
                            pred_score, pred_label = (probs_flat * label_class.unsqueeze(2)).max(1)
                            pred_label[pred_score < args.pred_region_th] = 255
                            feature_2d_label_onehot = F.one_hot(pred_label.view(-1), 256)[..., :args.num_classes].float()
                        else:
                        ...
                        if True:
                            ...
                            rank_pos_mask = (dot - dot.min(1, keepdim=True)[0]) * pos_mask
                    ...
                    if args.weight_pos:
                        with torch.no_grad():
                            ...
                            # tmp_weight = dot.max(1)[0].view(n, -1)
                            # tmp_weight = tmp_weight - tmp_weight.min(1, keepdim=True)[0]
                            # tmp_weight = tmp_weight / torch.clamp_min(tmp_weight.max(1, keepdim=True)[0], 1e-5)
                            # tmp_weight = tmp_weight.view(-1)
                    ...
                    #loss_mem = - ( torch.log(torch.clamp_min(logit_mem, 1e-5)) * pos_mask ).sum() / torch.clamp_min((pos_mask > 1e-5).float().sum(), 1e-5)
            ...
            if is_log and (batch % args.log_frequency == 0):
                ...
                examples_ = [v_img, v_real_gt, v_pred, v_seed_from_sp, v_gt, v_pix_feat_label, v_sp_feat_label, v_img_feat_label]
                ...
                if has_mem:
                    ...
                    if args.weight_pos:
                        v_tmp_weight = tmp_weight.view(n, h, w)[vid].data.cpu().numpy()
                        examples_.append(get_score_map(v_tmp_weight, v_img))
...
def run_test(gpu, args, port, verbose=False):
    ...
    # data
    dataset =  CSPointDataset(args.test_image_root, None, args.test_gt_root,
