...
def run_train(gpu, args, port):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch, return_vals in enumerate(loader, 1):
            ...
            ss_logit_softmax = torch.softmax(ss_logit, dim=1)
            ss_softmax_weight = ss_logit_softmax.max(1, keepdim=True)[0]
            ss_softmax_weight.detach_()
            ss_softmax_weight = ss_softmax_weight / ss_softmax_weight.sum(2, keepdim=True)
            ss_softmax_pool = (ss_softmax_weight * ss_logit_softmax).sum(2) # (n, c)
            loss_ss = F.binary_cross_entropy(ss_softmax_pool, label_cls)

            #ss_logit_pool = ss_logit.max(2)[0] # (n, c)
            #loss_ss = F.multilabel_soft_margin_loss(ss_logit_pool, label_cls)
            ...
            if is_log and (batch % args.log_frequency == 0):
                ...
                with torch.no_grad():
                    ...
                    sp_label_confidence_ = ss_softmax_weight[vid][0][sp_lookup]
...
def get_snapshot_name(args):
    ...
    dirname += '_ss{}'.format(args.lambda_ss)
    return dirname
...
if __name__ == '__main__':
    ...
    parser.add_argument('--snapshot', type=str, default='./snapshot/cam/cityscape')
    ...
    parser.add_argument('--ps-focal', type=float, default=0.)
