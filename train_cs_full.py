    ...
    for epoch in range(args.num_epochs):
        ...
        for batch, return_vals in enumerate(loader, 1):
            ...
            with torch.no_grad():
                ...
                sp_onehot_flat = sp_onehot.view(ns, hs * ws, args.max_superpixel)
                sp_size = sp_onehot_flat.sum(1, keepdim=True)

            nf, df, hf, wf = feature.size()
            feature_flat = feature.view(nf, df, hf * wf)
            sp_feature_ = torch.matmul(feature_flat, sp_onehot_flat) / torch.clamp_min(sp_size, 1e-3) # (n, d, nS)
            sp_feature = sp_feature_.detach()
            
            # pixel-to-segment loss
            mean_feature = torch.matmul(sp_feature, sp_onehot_flat.permute(0, 2, 1))
            if args.ps_type == 'mse':
                dist_ps = ((feature_flat - mean_feature)**2).sum(1)
            elif args.ps_type == 'dot':
                dist_ps = -(feature_flat * mean_feature).sum(1)
            spatial_mask_loss_ps = sp_onehot_flat.max(2)[0]

            # focal-loss-like weight
            with torch.no_grad():
                focal_weight = torch.exp((dist_ps - 1) * args.ps_focal)
                focal_weight = spatial_mask_loss_ps * focal_weight
                focal_weight = focal_weight / focal_weight.sum(1, keepdim=True)
                # print(focal_weight.max(1).data.cpu().numpy().ravel())

            #loss_ps = (dist_ps * spatial_mask_loss_ps).mean()
            loss_ps = (dist_ps * focal_weight).sum(1).mean()

            # segment-to-segment loss
            #ss_sim = torch.matmul(sp_feature.permute(0, 2, 1), feature_flat) # (n, nS, h*w)
            ss_sim = torch.matmul(sp_feature_.permute(0, 2, 1), sp_feature_) # (n, nS, nS)
            ss_sim_exp = torch.exp(ss_sim / args.ss_temp)
            with torch.no_grad():
                label_prob = F.softmax(logit_seg, 1).view(nf, args.num_classes, hf * wf)

                # pixel pseudo-label
                #feature_label_confidence, feature_label = label_prob.max(1)
                #feature_label[feature_label_confidence < args.feature_label_th] = args.num_classes
                #feature_label_onehot = F.one_hot(feature_label, args.num_classes+1)[:, :, :args.num_classes] # (n, h*w, c)

                # segment pseudo-label
                sp_label_prob = torch.matmul(label_prob, sp_onehot_flat) / torch.clamp_min(sp_size, 1e-3) # (n, c, nS)
                sp_label_confidence, sp_label = sp_label_prob.max(1)
                sp_label[sp_label_confidence <= args.sp_label_th] = 255
                sp_label_onehot = F.one_hot(sp_label, 256)[..., :args.num_classes].to(torch.float32) # (n, nS, c)
                sp_label_exists = (sp_label < args.num_classes).to(torch.float32) # (n, nS)

                ss_pos = torch.matmul(sp_label_onehot, sp_label_onehot.permute(0, 2, 1))
                ss_neg = (1 - ss_pos) * (sp_label_exists.unsqueeze(1) * sp_label_exists.unsqueeze(2))
                non_self_mask = 1 - torch.eye(args.max_superpixel, dtype=torch.float32, device=gpu)
                ss_pos = ss_pos * non_self_mask
                ss_neg = ss_neg * non_self_mask

            ss_contrast = ss_sim_exp / (ss_sim_exp * (ss_pos + ss_neg) + 1e-5)
            loss_ss = - ( torch.log(torch.clamp_min(ss_contrast, 1e-10)) * ss_pos ).sum(2) / torch.clamp_min(ss_pos.sum(2), 1)
            loss_ss = loss_ss.mean()

            loss = loss_ce + loss_ps * args.lambda_ps + loss_ss * args.lambda_ss
            loss.backward()
            optimizer.step()
            ...
            if is_log and (batch % args.log_frequency == 0):
                ...
                with torch.no_grad():
                    ...
                    sp_label_confidence_ = sp_label_confidence[vid][sp_lookup]
                    sp_label_ = sp_label[vid][sp_lookup]
                    ...
                    num_ss_pos = ss_pos.sum(2).data.cpu().numpy().mean()
                    num_ss_neg = ss_neg.sum(2).data.cpu().numpy().mean()
                    print(num_ss_pos, num_ss_neg)
                ...
                v_dist_ps = get_score_map(dist_ps[vid].data.cpu().numpy().reshape(hf, wf), v_img)

                # segment-to-segment
                v_sp_conf = get_score_map(sp_label_confidence_, v_img)
                v_sp_label = get_segment_map(sp_label_)

                examples_ = [v_img, v_real_gt, v_gt, v_pred, v_sp, v_dist_ps, v_sp_conf, v_sp_label]
...
def get_snapshot_name(args):
    ...
    dirname += '_{}_focal{}_ps{}'.format(args.ps_type, args.ps_focal, args.lambda_ps)
    dirname += '_t{}_spth{}_ss{}'.format(args.ss_temp, args.sp_label_th, args.lambda_ss)
    return dirname
...
if __name__ == '__main__':
    ...
    parser.add_argument('--snapshot', type=str, default='./snapshot/pixel_to_segment/cityscape')
    ...
    parser.add_argument('--concat-layers', type=str, default='conv5,fc6,fc7')
    ...
    parser.add_argument('--ps-type', type=str, default='dot')
    parser.add_argument('--ps-focal', type=float, default=1.)

    parser.add_argument('--ss-temp', type=float, default=1.0)
    parser.add_argument('--sp-label-th', type=float, default=0.7)
    parser.add_argument('--lambda-ss', type=float, default=0)
