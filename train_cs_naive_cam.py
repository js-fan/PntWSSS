...
def run_train(gpu, args, port):
    ...
    # model
    mod = eval(args.model)(args.num_classes, dilation=args.dilation, upsample=args.upsample,
            embedding=args.embedding, concat_layers=args.concat_layers.split(',')).to(gpu)
    if args.resume:
        pretrained = os.path.join(args.snapshot, 'checkpoint', '%s-%04d.pth' % (args.model, args.resume-1))
    ...
    # train
    for epoch in range(args.resume, args.num_epochs):
        # reset loader
        if is_distributed:
...
def run_cam(gpu, args, port, verbose=False):
    ...
    get_save_src_np = lambda x: os.path.join(args.snapshot, 'results', 'cam',
    ...
    for i, (image, label, src) in enumerate(loader):
        ...
        if (gpu == 0) and (i < 20):
            npsave(get_save_src_np(src[0]), pred_logits)
        ...
        imwrite(get_save_src(src[0]), cam_label)
    ...
    if (gpu == 0):
        ...
        examples = imvstack(examples)
        imwrite(os.path.join(args.snapshot, 'results', 'image_gt_seed_cam_{}.jpg'.format(args.cam_threshold)), examples)
...
def get_snapshot_name(args):
    ...
    dirname += '_dilat{}'.format(args.dilation)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cam-threshold', type=int, default=0.3)
