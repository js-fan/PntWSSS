...
def train(cfg):
    ...
    mod.init_params(cfg.model.get('pretrained', None))
    ...
    # Train
    niter = 0
    rampup_iters = cfg.hyperparam.rampup_iters
    get_rampup = lambda xiter: 1 if (rampup_iters == 0) else (min(xiter, rampup_iters) / rampup_iters)
    for epoch in range(cfg.schedule.num_epochs):
        ...
        for data in loader:
            ...
            with torch.no_grad():
                sp_pred = average_sp(torch.softmax(logit, 1), sp_mask)
            ...
            loss_pl_main = criteria_ce(logit, sp_label2d)
            loss_pl_aux = criteria_ce(logit_aux, sp_label2d)


            loss_base = loss_main + loss_aux * cfg.hyperparam.aux_weight
            loss_pl = loss_pl_main + loss_pl_aux * cfg.hyperparam.aux_weight
            loss = loss_base + loss_pl * get_rampup(niter)
            loss.backward()
            optimizer.step()
            niter += 1
            ...
            if rank == 0:
                ...
                if niter % cfg.log_freq == 0:
                    ...
                    logger.push("Lbase", loss_base.item())
                    logger.push("Lpl", loss_pl.item())
                    logger.push("rampup", get_rampup(niter))
                    logger.flush(f"Epoch={epoch}, Iter=[{niter}/{total_iter}]")

                    vid = niter % image.shape[0]
                    demo = auto_make_demo(image[vid], label[vid], sp_label2d[vid], logit[vid], logit_aux[vid])
