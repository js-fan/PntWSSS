...
def run_train(gpu, args, port):
    ...
    # sync-bn
    if args.model == 'deeplab_v3p':
        mod = nn.SyncBatchNorm.convert_sync_batchnorm(mod)
    elif args.model == 'deeplab_v2':
        mod = nn.SyncBatchNorm.convert_sync_batchnorm(mod)
        #mod.freeze_bn()
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch, return_vals in enumerate(loader, 1):
            ...
            _, _, h, w = logit_seg.size()
            n, d, fh, fw = feature.size()

            feature = feature / (torch.norm(feature, dim=1, keepdim=True) + 1e-5)
            feature_flat = feature.view(n, d, fh * fw)
            ...
            if (h != fh) or (w != fw):
                with torch.no_grad():
                    logit_seg_dw = F.interpolate(logit_seg, (fh, fw), mode='bilinear')
                    logit_flat_dw = logit_seg_dw.view(n, args.num_classes, fh * fw)
                    probs_flat_dw = torch.softmax(logit_flat_dw, 1)
            else:
                logit_seg_dw = logit_seg.detach()
                logit_flat_dw = logit_flat.detach()
                probs_flat_dw = probs_flat.detach()
            ...
            with torch.no_grad():
                # pixel-level feature
                pix_feat = feature_flat.permute(0, 2, 1).reshape(n * fh * fw, d)
                pix_feat_flat = pix_feat
                pix_feat_prob, pix_feat_label = (probs_flat_dw * label_class.unsqueeze(2)).max(1)
                pix_feat_label[pix_feat_prob < 0.9] = 255
                pix_feat_label = pix_feat_label.view(-1)

                # superpixel-level feature
                if (h != fh) or (w != fw):
                    superpixel_onehot_dw = F.interpolate(superpixel_onehot.view(n, h, w, args.max_superpixel).permute(0, 3, 1, 2),
                            (fh, fw), mode='bilinear').permute(0, 2, 3, 1).view(n, fh * fw, args.max_superpixel)
                else:
                    superpixel_onehot_dw = superpixel_onehot
                superpixel_size_dw = torch.clamp_min(superpixel_onehot_dw.sum(1), 1e-5)

                sp_feat = torch.matmul(feature_flat, superpixel_onehot_dw) / superpixel_size_dw.unsqueeze(1) # (n, d, s)
                ...
                some_feat_label_onehot = F.one_hot(some_label.view(n, -1), 256)[..., :args.num_classes].float() # (n, fh*fw, c)
            ...
            if has_mem:
                ...
                for deq_feat, deq_label in deq_results:
                    q = deq_feat.size(0)
                    feature_2d = feature_flat.permute(0, 2, 1).reshape(n * fh * fw, d)
                    dot = torch.matmul(feature_2d, deq_feat.T) # (n*fh*fw, q)
                    dot_exp = torch.exp(dot / args.temperature)

                    # pos, neg masks
                    with torch.no_grad():
                        if (h != fh) or (w != fw):
                            feature_2d_label = F.interpolate(seed_from_sp.unsqueeze(1).float(), (fh, fw), mode='nearest').squeeze(1).long()
                        else:
                            feature_2d_label = seed_from_sp
                        feature_2d_label_onehot = F.one_hot(feature_2d_label.view(-1), 256)[..., :args.num_classes].float() # (n*fh*fw, c)
                        deq_label_onehot = F.one_hot(deq_label, args.num_classes).float() # (q, c)

                        pos_mask = torch.matmul(feature_2d_label_onehot, deq_label_onehot.T) # (n*fh*fw, q)
                        ...
                        if args.hard_example_mining:
                            ...
                            if args.num_neg > 0:
                                ...
                                if args.numpy_hdem:
                                    ...
                                    neg_mask = F.one_hot(rank_neg_mask_topk, q).to(torch.float32).sum(1)[0] * neg_mask # (n*fh*fw, q)
                            else:
            ...
            if is_log and (batch % args.log_frequency == 0):
                ...
                if has_mem:
                    ...
                    meter.put('nPos', pos_mask.sum().item()/(n * fh * fw))
                    meter.put('nNeg', neg_mask.sum().item()/(n * fh * fw))
                ...
                with torch.no_grad():
                    ...
                    # CL
                    if has_mem:
                        dot_most_sim_val, dot_most_sim_id = dot.view(n, fh, fw, q)[vid].max(2) # (fh, fw)
                        dot_most_sim_cls = deq_label[dot_most_sim_id.view(-1)].view(fh, fw)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='deeplab_v3p')
    parser.add_argument('--train-size', type=str, default='513,513')
    ...
    parser.add_argument('--snapshot', type=str, default='./snapshot/abl3/voc')
